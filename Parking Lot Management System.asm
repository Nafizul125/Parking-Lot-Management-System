...
.DATA
    ; --- Feature 3: Slot allocation using arrays ---
    parking      DW 10 DUP(0)    ; Array to store vehicle numbers (0 = empty)
    start_hours  DB 10 DUP(0)    ; Array to store entry hours for each slot
    ...
    ; --- Feature 5 & 6: Calculation Outputs ---
...
main_loop:
    ...
    JE choice2
    CMP AL, '3'
    JE choice3 ; New Feature: Exit System
    CMP AL, '4'
    JE choice4
...
; --- Feature 1: Vehicle Entry System ---
vehicle_entry PROC
    LEA DX, prompt_veh
    MOV AH, 09H
    INT 21H
...
; --- Feature 2: Slot Availability Display ---
slot_status PROC
    LEA DX, slot_header
    MOV AH, 09H
    INT 21H
...
; --- Feature 4, 5, & 6: Vehicle Exit, Duration & Fee ---
vehicle_exit PROC
    LEA DX, prompt_slot
    MOV AH, 09H
    INT 21H
    ...
    ; Logic: Exit Time must be > Entry Time
    MOV SI, BX
    MOV AL, exit_hour
    SUB AL, start_hours[SI]
    JS time_error ; Jump if Sign (Negative)
    
    MOV AH, 0
    MOV duration, AX
    
    ; Fee calculation (50 TK per hour)
    MOV DX, 50
    MUL DX
    MOV total_fee, AX
...
; Print multi-digit number in AX
print_number PROC
    PUSH AX
    PUSH BX
    PUSH CX
